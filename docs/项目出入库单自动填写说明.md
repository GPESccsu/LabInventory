...
python inv.py --db <数据库路径> proj-forms \
  ...
  [--lcsc-file <立创导出文件>] \
  [--inbound-loc <入库库位>] [--no-auto-inbound]
```
...
- `--apply-inbound`：将入库单数量直接写入库存（兼容旧参数）
- `--no-auto-inbound`：即使提供 `--lcsc-file` 也不写库存
- `--inbound-loc`：入库库位；不填时自动取 `locations` 表第一个库位，若 `locations` 为空则自动创建 `AUTO-INBOUND`
